...
const uploadSingleJson = async (
  ...
  try {
    ...
    const createdAsteroids = await Asteroid.bulkCreate(asteroidBulkData, {
      transaction: transaction,
    ...
    jsonOld.asteroids.forEach((asteroid) => {
      ...
      asteroid["geysers"].forEach((geyser) => {
        if (totalGeyserOutputBulkData[i][`${geyser.id}_Count`]) {
          ...
          totalGeyserOutputBulkData[i][`${geyser.id}_TotalOutput`] +=
            geyser["avgEmitRate"];
        } else {
          //new geyser
    ...
    gs = await TotalGeyserOutput.bulkCreate(totalGeyserOutputBulkData, {
      transaction: transaction,
      individualHooks: true,
      validate: true,
      hooks: true,
    ...
    await TotalGeyserOutput.create(clusterGeyserOutput, {
      transaction: transaction,
    });
    ...
    if (printClusters)
      console.log(
        `${numtoPrint}: New Cluster uploaded: ${newCluster.coordinate}`
      );
...
const uploadBulk = async (jsonArrOld, printClusters = true) => {
  ...
  for (let i = 1; i <= jsonArrOld.length; i++) {
    ...
    queue.push(
      myq.run(() =>
        uploadSingleJson(jsonOld, i, printClusters).catch((err) =>
          console.error(err)
        )
      )
    );
...
/* sync force true (line 166) overrides and drops current database
 ...
 Upload is used to add seeds to the SQL database. 
 */
...
router.post("/one", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ response: "Upload failed", error: error.message });
  }
});
...
router.post("/refreshSQL", async (req, res) => {
  //const inputFilePath = "./utils/Worlds/data-1.json";
  const inputFiles = [
    "./utils/Worlds/data-1.json",
    "./utils/Worlds/data-2.json",
    "./utils/Worlds/data-3.json",
    "./utils/Worlds/data-4.json",
    "./utils/Worlds/data-5.json",
    "./utils/Worlds/data-6.json",
    "./utils/Worlds/data-7.json",
    "./utils/Worlds/data-8.json",
    "./utils/Worlds/data-9.json",
    "./utils/Worlds/data-10.json",
    "./utils/Worlds/data-11.json",
    "./utils/Worlds/data-12.json",
    "./utils/Worlds/data-13.json",
    "./utils/Worlds/data-14.json",
  ];
  console.log(
    "-------------------------------Starting SQL Refresh------------------------------"
  );
  ...
  for (const inputFilePath of inputFiles) {
    ...
    console.log(
      `There are ${jsonArray.length} seeds in ${inputFilePath} about to be uploaded.`
    );
